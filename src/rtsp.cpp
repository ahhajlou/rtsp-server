...
void Rtsp::accept(void) {
    ...
    for (;;) {
        std::thread(connection, _acceptor.accept()).detach();
    }
}

void Rtsp::connection(tcp::socket sock) {
    RtspConnection(std::move(sock)).loop();
    ...
    //                               : make_error_response(event_result.error(),
    //                               cseq).serialize(cseq);
